...
def main():
    ...
    save_model(
        model, 
        os.path.join(download_dir, 'resnet50_corgi_classifier.pth'),
        class_names=class_names,
        history=history
    )

    # Example of loading the model back
    loaded_model, checkpoint = load_model(os.path.join(download_dir, 'resnet50_corgi_classifier.pth'))
    
    if loaded_model:
        print("Successfully loaded the model")
        if checkpoint and 'classes' in checkpoint:
